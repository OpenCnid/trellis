...
1. `trellis_neo4j`: a read-only Neo4j wrapper.
   ...
   - The ONLY permitted writes are `trellis_neo4j.write_derived_insight(subject, verb, obj, sourceNodeIds, confidence=None)`, which caches a deduced fact as a DERIVED_INSIGHT edge with spatial provenance, and its bulk variant `trellis_neo4j.write_derived_insights(facts)` which writes a whole list of fact dicts (keys: subject, verb, obj, sourceNodeIds, confidence) in ONE round trip. `confidence` (0.0-1.0) is the sub-LLM's self-reported probability for the fact; always pass it through when the sub-LLM provided one.
