...
| 13 | *Test-Time Learning for LLMs*, arXiv:2505.20633 | LoRA mitigates catastrophic forgetting more than full fine-tuning; the drift-bound citation |
...
Three items post-dating the §10 reading list, logged here because they sharpen the R3/R4 criterion and the estimate basis; no gate moves.
...
2. LoRA and catastrophic forgetting (arXiv:2505.20633). A separate result that at test time LoRA mitigates catastrophic forgetting more than full-parameter updates. Recorded as complementary, not conflated with item 1: the cost-Pareto study does not measure forgetting (it defers retention to future work), so the drift-bound property rests on this citation alone. If an R4 arm is instantiated, this is the citation behind preferring a low-rank adapter as the retention-bounding choice.

3. Workspace manipulation as a potential avenue of investigation (not a rung). The global-workspace / Jacobian-lens result (Gurnee et al., transformer-circuits.pub/2026/workspace) identifies a small, causally-privileged subspace a residual-stream read can inspect, with community reproductions on small open checkpoints (Qwen3-4B, Gemma-2-2B, Llama-3.2-3B). It suggests a possible avenue: manipulating that subspace to strengthen meta-prompt adherence, i.e. H2's mechanism approached through measurement rather than fast weights. Recorded as an avenue only; out of scope for R2 through R5, no criterion attached, no claim, subject to the same paired-arm and owner-gated discipline as everything else here should it ever be taken up.
