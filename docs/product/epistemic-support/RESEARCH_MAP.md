...
(S1–S9), 27 claims (R-01…R-27), 11 adoption bounds (AB-1…AB-11, §9).
...
mirrors), 5 repo observations, 4 secondary syntheses, 1 attribution
record (verified artifact dates), 1 background-theory hypothesis
awaiting its source (R-14/S6). Companion artifacts: the parent design record
...
| 9 | S9 full paper + released code | paper mirror `github.com/OpenCnid/better-harnesses-smaller-models-pdf` (checksum-verified); **code locator FOUND in the primary: `github.com/malusamayo/migration-analysis`** (footnote 1) | R-25…R-27; the failure-taxonomy and optimizer designs for any future Trellis adaptation work | paper (have); code: selected files (task suite, optimizer loop, failure-diagnosis prompts) | code license **unverified** — check before vendoring anything | paper `web:https://arxiv.org/abs/2607.08938`; code `repo:migration-analysis:<path>` if adopted | operator promotion for the paper approved-in-pattern; code acquisition NOT yet owner-approved — propose before fetching for reuse | optimizer implementation details beyond the paper's prose |
