...
**Register summary (as of July 16, 2026, late session):** 9 sources
(S1–S9), 26 claims (R-01…R-26), 11 adoption bounds (AB-1…AB-11, §9).
By evidence class: 15 primary findings (9 from S1; 4 from S8,
primary-verified; 2 from S9, coverage-derived pending the primary),
...
| S9 | "Better Harnesses, Smaller Models: Building 90% Cheaper Agents via Automated Harness Adaptation" | Chenyang Yang, Xinran Zhao, Tongshuang Wu, Christian Kästner | arXiv:2607.08938 (July 2026) | Primary — **coverage-derived** (abstract + reported results via search; primary text blocked by this session's network policy; the S8 mirror pattern is the upgrade path) | Abstract-level only | Absent | arXiv publication; owner designated it (July 16, 2026) the **overarching guide to Trellis's purpose** for the next handoff |
...
| R-25 | n/a (purpose framing) | none | Adopt as the handoff's purpose statement: Trellis = RLM depth × adaptive harness, with uncertainty-around-facts managed by the adaptive four-judge layer; obtain the primary (S8 mirror pattern) before any mechanism-level reliance; any *automated* harness adaptation in Trellis enters behind AB-8 (anchors and human gates first — the failure-trajectory optimizer is S1's metric loop pointed at the harness, and inherits the same guards) | Primary text contradicting the coverage; or replication failure of the harness-lift result |
