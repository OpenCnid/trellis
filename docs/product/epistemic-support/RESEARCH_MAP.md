...
| S7 | WonderSuite 2.0 — specifically WonderScholar 2.0 ("Topological Research Framework") and WonderBuild 2.0 ("Adaptive Knowledge Construction Framework") | gusthemole / Lexideck Technologies (same lineage as S2/S3) | https://github.com/gusthemole/WonderSuite, `WonderScholar.txt` + `WonderBuild.txt`; version 2.0; repo history March 2, 2025 → January 7, 2026 (verified via commit log); retrieval date July 16, 2026. **AMENDED July 22, 2026 (corpus study, §7 of `docs/RESEARCH_NOTES_COLLECTION.md`):** pinned at commit `092037f` (2026-01-07), re-verified 2026-07-22 against a full local clone. **Date-range correction flagged, not applied:** the row's start date is one day late — the first two commits (`4a32f88` LICENSE, `321b04d` all six `.txt` files) are both dated **2025-03-01**; `2025-03-02` is `3d9b167`, a later README pass. Correcting a governed row is an owner act by dated entry, so the original text stands above with this receipt beside it | Secondary (conceptual framework, prompt-layer, unmeasured) | Full text (fetched raw files) | Absent as source. **A locator-verified corpus study exists outside this repo** (July 22, 2026): [`OpenCnid/axes-of-wonder`](https://github.com/OpenCnid/axes-of-wonder) — mirror plus a four-branch density-chain map and SPARK diagnosis. It is a **secondary source and does not change this row's evidence class or standing** (`RESEARCH_NOTES_COLLECTION.md` §7.1); it does not satisfy an obligation to read the primary. Bearing on this row: only three of five engines declare Dimensional Axes, and **WonderScholar and WonderBuild — the two this row names — are both in that three**, so the row's "Topological Research Framework"/"Adaptive Knowledge Construction Framework" characterization is sound for its named subjects even though the upstream README over-claims the approach for all five | Public, **GPL v3** — cite concepts and short excerpts; do not vendor GPL text into this MIT repo |
