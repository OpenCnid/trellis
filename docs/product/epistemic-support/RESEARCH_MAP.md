...
|---|---|---|---|---|---|---|---|
| S1 | "Who Grades the Grader? Co-Evolving Evaluation Metrics and Skills for Self-Improving LLM Agents" | Xing Zhang, Guanghui Wang, Yanwei Cui, Ziyuan Li, Wei Qiu, Bing Zhu, Peiyang He — AWS Generative AI Innovation Center + HSBC Technology Center China | arXiv:2607.12790v1, July 14, 2026; durable mirror `github.com/OpenCnid/who-grades-the-grader-pdf` (SHA-256 `cf7aca47…b05f` verified July 16, 2026 — byte-identical to the copy read in session) **[Amended July 18, 2026: repo renamed `who-grades-the-grader`; PDF removed from HEAD by owner ruling (bytes remain in git history; the hash still verifies them); the repo now carries a locator-verified five-tier note pinned to v1]** | Primary | Full text (13 pp., read in full) | Note repo under the OpenCnid org; not ingested/promoted | Repo CITATION.md directs citation to the original (DOI 10.48550/arXiv.2607.12790) |
...
| S8 | "Verbalizable Representations Form a Global Workspace in Language Models" (the "J-space" / Jacobian-lens paper) | Wes Gurnee, Nicholas Sofroniew, Adam Pearce, et al. (16 authors), Anthropic | https://transformer-circuits.pub/2026/workspace/, published July 6, 2026; **full 124-page text read July 16, 2026** from the collaborator-provided mirror `github.com/OpenCnid/verbalizable-global-workspace-pdf` (SHA-256 verified: `fc0f49de…bbae` matches the mirror's declared checksum) **[Amended July 18, 2026: the mirror was renamed `verbalizable-global-workspace`, its PDF removed from HEAD by owner ruling (bytes in git history; hash still verifies), and the repo slated for deletion; durable pointers are the canonical page and the note repo `github.com/OpenCnid/global-workspace-in-llms` (locator-verified five-tier note, verified against the live page July 18, 2026). The July 16 read event stands as history]** | Primary (mechanistic interpretability) — **primary-verified**; earlier coverage-derived readings upgraded/corrected in R-20/R-21 below | Full text | Note repo `global-workspace-in-llms` under the OpenCnid org; not ingested/promoted into Trellis | Public research publication; the note repo's CITATION.md directs citation to the original |
| S9 | "Better Harnesses, Smaller Models: Building 90% Cheaper Agents via Automated Harness Adaptation" | Chenyang Yang, Xinran Zhao, Tongshuang Wu, Christian Kästner | arXiv:2607.08938v1 (submitted July 9, 2026), CMU; durable mirror `github.com/OpenCnid/better-harnesses-smaller-models-pdf` (SHA-256 `3fac74f3…3b17` verified; **full 12-page text read July 16, 2026**) **[Amended July 18, 2026: repo is `better-harnesses-smaller-models` (the `-pdf` name was already retired); PDF removed from HEAD by owner ruling (bytes in git history; hash still verifies); the repo now carries a locator-verified five-tier note pinned to v1]** | Primary — **primary-verified** (upgraded same day from coverage-derived) | Full text | Note repo under the OpenCnid org; not ingested/promoted | Repo CITATION.md directs citation to the original (DOI 10.48550/arXiv.2607.08938); paper's own code locator: `github.com/malusamayo/migration-analysis` (footnote 1) |
...
| S11 | "Polymorphic Combinatorial Frameworks (PCF): Guiding the Design of Mathematically-Grounded, Adaptive AI Agents" | **David Pearl, Matthew Murphy, James Intriligator** (co-authorship verified via multi-query coverage July 16, 2026) | arXiv:2508.01581, submitted August 3, 2025 (predates S8 by ~11 months); earlier Research Square preprint exists (DOI 10.21203/rs.3.rs-6397317/v1) **[Amended July 18, 2026: the lab's note repo `github.com/OpenCnid/pcf-adaptive-agents` carries a five-tier density-chain note on this paper; a note is a secondary source and does not change this row's full-text-unread standing — register reads require primary verification]** | Primary — **coverage-verified metadata only; full text unread** (arxiv proxy-blocked; mirror pattern is the acquisition path) | Abstract-level coverage | Absent | Coverage-verified abstract content: LLM-guided meta-prompt design over mathematically-grounded combinatorial spaces; combinatorial logic, topos theory, **rough fuzzy set theory**; the SPARK parameter space (Skills, Personalities, Approaches, Resources, Knowledge); 1.25M Monte Carlo simulations. Per collaborator (July 16): PCF was cited **as a courtesy to its co-author**; UHE was coined in separate, non-public independent research and is **not expected in PCF's text — do not hunt the term**. Do not canonize PCF's math until read |
| S12 | *Emotion Concepts and their Function in a Large Language Model* (Anthropic; Sonnet 4.5) — collaborator-cited as supporting the **vocalize-ability** of the Emotional plane (no phenomenology claimed by any party). **Dated entry July 18, 2026:** locator recovered and row updated after verifying the read against the sidecar record — this is the owner-supplied deep-dive paper of the (since-resolved) HANDOFF §3 research objective | Anthropic | transformer-circuits.pub/2026/emotions (published April 2, 2026); mirror present under the machine convention at `D:\OpenCnid\emotion-concepts-in-llms` **[Amended July 18, 2026: the GitHub repo `github.com/OpenCnid/emotion-concepts-in-llms` now carries a locator-verified five-tier density-chain note (pinned arXiv:2604.07729v1, verified against a fresh fetch July 18, 2026); the local path is a machine-local clone of that repo]**; **read July 17, 2026 in the owner session** that produced [`RESIDUAL_STREAM_SIDECAR.md`](../../architecture/RESIDUAL_STREAM_SIDECAR.md) — its §2 distills the five findings leaned on, and its claims table types them (linear/causal/dose-responsive directions MEASURED (external); axis generality EXTRAPOLATED) | Primary (mechanistic interpretability) — read; per-claim evidence classes live in the sidecar record, not here | Full text (owner session; no in-repo checksum recorded for the read copy — noted, not invented) | Absent — mirror only, not ingested/promoted | Public research publication; cite the original. The HANDOFF §3 pre-registered condensation-expectation verdict against this paper remains OPEN, owner-owed |
...
| R-33 | S13 §11.4 ("at threshold a particle can *be*; it cannot *do*") + §9 | Existence and interaction-realizability are different budgets: at its own threshold an entity holds exactly the cost of its existence and has no surplus to fund an exchange | SS, converging with enforced repo doctrine | `AMBIENT.md` rule 15 (correct is not the same claim as reachable) — the repo's four correct-but-unreachable instances are entities verified at the existence budget with no interaction surplus (no non-test caller) | `AMBIENT.md` rule 15; the EL-11 reachability rule | Converges — names the mechanism behind an existing hard rule |
...
**4.9 Reachability, stated up front.** [E] `AMBIENT.md` rule 15:
...
*(Pointer, July 18, 2026: the org-level acquisition and note conventions
this queue relies on — the note-repo inventory, the machine face
(`index.json`), the authority ordering, and the PROPOSED
ingestion/staleness contract — are recorded in
...
|---|---|---|---|---|---|---|---|---|
| 1 | S1 full paper | arXiv:2607.12790; durable mirror `github.com/OpenCnid/who-grades-the-grader-pdf` (checksum-verified) **[Amended July 18, 2026: mirror retired by owner ruling — repo renamed `who-grades-the-grader`, PDF in git history only (hash still verifies); acquisition path for ingestion is the arXiv v1 fetch or the historical commit]** | R-01…R-09 | full text (read; snapshot in repo history) | repo CITATION.md governs | `web:https://arxiv.org/abs/2607.12790` | **OWNER-APPROVED July 16, 2026** — ingestion pending a durable deployment | nothing — acquisition complete (fetch-on-demand); promotion is the remaining step |
...
| AB-8 | ~~No evolution/search machinery in first editions — hand-authored op pools and judge configurations under anchor discipline~~ **AMENDED July 19, 2026 (owner ruling, Session 71): "hand-authored" conflated two different things and blocked the design.** The bound now separates them. (a) **Composition is permitted and is the design** — judges, their selections, taxonomies, names and anchors compose per context at ceremony time from the REPL's own fact and belief space (JUDGE_COMPOSITION_GAME §6 rule 4 as superseded; JUDGE_COMPOSITION_CEREMONY). (b) **Evolution, search, and optimization over compositions remain gated** — no automated variation-and-selection of judges, no metric-driven tuning of them, without its own proposal. The original's evidence basis (R-02's vacuous collapse, R-03's anchor drift) is about unvalidated automated search, not about composing for a context; R-02's actual protection is the validity gate (refuse all-pass/all-fail/all-abstain), which is taxonomy-agnostic and **rehomes intact onto composition time**, where it runs against composed anchors before any judging. | R-02, R-03, §4.5; amended by owner ruling July 19, 2026 |
...
| AB-11 | Corroboration and judging read **live blocks only** (superseded versions are archive, not search space) | `.claude/rules/substrate-writes.md` rule 13, parent §5 |
