...
* Acts are individually runnable (`npm run drill:update -- --acts 2,3` is LLM-free); `npm run drill:reset -- --confirm-reset` clears the drill's registry versions and semantic leftovers so the drill can re-run from Act 1 (`oolong:ingest` + `oolong:flywheel-prep -- --confirm-strip` restore the v1 graph).
* Every drill entrypoint refuses a database that carries no drill-target marker, and every destructive one additionally refuses without its `--confirm-*` flag ([drill_target.ts](../../src/core/runtime/drill_target.ts)). Declare a scratch stack expendable once with `npm run drill:mark-target`; `npm run test:drill-gate -- --negative-control` exercises all four refusal paths.
