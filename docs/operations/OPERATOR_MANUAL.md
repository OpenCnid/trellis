...
> rules. Human contributors: [`CONTRIBUTING.md`](../../.github/CONTRIBUTING.md) and
> [`docs/README.md`](../README.md) (the documentation reading order).
...
1. **The substrate (what "GraphRAG" named).** Every source — a PDF, a
   ...
   F1 = 1.000 ([UPDATE_DRILL_REPORT](../benchmarks/UPDATE_DRILL_REPORT.md)).
...
4. **The two flywheels (knowledge, then capability).** The **knowledge
   ...
   [FLYWHEEL_EXPLAINER](../benchmarks/FLYWHEEL_EXPLAINER.md)). The
...
5. **The discipline (code-mediated text, the core pillar).** *The model
   ...
   ([CODE_MEDIATED_TEXT](../architecture/CODE_MEDIATED_TEXT.md)).
...
6. **Composition from primitives (the *composable* in "composable expert
   ...
   **modules** that shape a run — each is composed *per context* from
   ...
   ([COMPOSITION_FROM_PRIMITIVES](../architecture/COMPOSITION_FROM_PRIMITIVES.md)).
...
[JUDGE_COMPOSITION_GAME](../product/epistemic-support/JUDGE_COMPOSITION_GAME.md);
the general principle:
[COMPOSITION_FROM_PRIMITIVES](../architecture/COMPOSITION_FROM_PRIMITIVES.md);
the outsider briefing:
[COLLABORATOR_BRIEFING](../COLLABORATOR_BRIEFING.md).
...
([STANDING_MODEL](../product/epistemic-support/STANDING_MODEL.md),
...
[EPISTEMIC_SUPPORT](../architecture/EPISTEMIC_SUPPORT.md); program
documents: [docs/product/epistemic-support/](../product/epistemic-support/PROGRAM_CONTEXT.md).
...
([DOUBTS_WORKSPACE](../architecture/DOUBTS_WORKSPACE.md), proposed).
...
Authority is explicit — **code > glossary > prose**. Agents load root
`AGENTS.md` first and use [docs/ORIENTATION.md](../ORIENTATION.md) for the
system model; the collaborator's live task supplies current scope. Canonical terms live in
[docs/GLOSSARY.md](../GLOSSARY.md); doctrine and forward design in
[docs/architecture/](../architecture/) (`WORKSPACE_AND_MODULES.md`,
`CODE_MEDIATED_TEXT.md`, `GROUNDED_AUTHORING.md`); measured evidence in
[docs/benchmarks/](../benchmarks/); history in [docs/product/](../product/).
The reading map is [docs/README.md](../README.md).
...
metric labels or log content. See `docs/reference/API_REFERENCE.md` §0 and
...
happens by default. See `docs/reference/API_REFERENCE.md`.
...
See `docs/reference/API_REFERENCE.md` §4 for the event contract.
...
See `docs/reference/API_REFERENCE.md` §5 for the wire contract.
...
- `data/oolong_pairs_dataset.json` — v1 (`oolong-pairs-trec-synthetic-v1`),
  the saturated baseline behind the committed
  `docs/benchmarks/artifacts/benchmark_results.json`.
...
overwrite the committed v1 `docs/benchmarks/artifacts/benchmark_results.json`
with another corpus's
...
behavior after 12 re-ingests, writes
`docs/benchmarks/artifacts/scale_drill_results.json`, and removes
...
§6.3 measurement across three rounds: the committed Frankenstein corpus,
...
See [API_REFERENCE.md](../reference/API_REFERENCE.md) for endpoint contracts,
[docs/README.md](../README.md) for the documentation map, and
[docs/ORIENTATION.md](../ORIENTATION.md) for the architectural mental model.
