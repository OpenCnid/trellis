// Wrapper for scripts/probe_workspace_lineage.py — the Session 16
// follow-up to the design-record §11 step-1 probe: the paired protocol
// across a two-task goal, measuring whether SEEDED workspaces (design
// record §5 lineage) eliminate cross-task re-derivation. PAID: requires
// OPENAI_API_KEY and makes real LLM calls (owner-approved per run).
// Deliberately has no npm script alias and is not part of any acceptance
// suite; run explicitly with `tsx scripts/probe_workspace_lineage.ts`.
// The only MCP server involved is the local deterministic fixture over
// stdio; the databases are the local dev stack.
import { spawn } from 'child_process';
import path from 'path';
import { config } from '../src/config/index';
import { parseMcpServers, serializeMcpServers } from '../src/config/mcp_servers';

const script = path.resolve('scripts/probe_workspace_lineage.py');
